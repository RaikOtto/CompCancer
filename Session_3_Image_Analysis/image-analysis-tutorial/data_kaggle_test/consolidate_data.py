...
def work(args, sample):
    ...
    if raw.shape[0] < 360 or raw.shape[1] < 360:
        return
    ...
    gt_labels = gt_labels[:360, :360]
    gt_labels_tmp = np.zeros_like(gt_labels)
    ...
    for label in np.unique(gt_labels):
        ...
        dilated_label_mask = scipy.ndimage.binary_dilation(label_mask,
                                                           ...
                                                           structure=struct,
                                                           border_value=1)
        eroded_label_mask = scipy.ndimage.binary_erosion(dilated_label_mask,
                                                         iterations=1,
                                                         structure=struct,
                                                         border_value=1)
        gt_labels_tmp[eroded_label_mask] = label
    gt_labels = gt_labels_tmp

    gt_labels = pad(args, gt_labels, 'constant')

    gt_labels_tmp = np.zeros((gt_labels.shape[0]+1,
                              gt_labels.shape[1]+1), dtype=np.uint16)
    gt_labels_tmp[1:, 1:] = gt_labels

    gt_affs = np.zeros((2,) + gt_labels.shape, dtype=np.uint8)
    gt_affs[0, ...] = gt_labels == gt_labels_tmp[0:-1, 1:]
    gt_affs[0, ...][gt_labels == 0] = 0
    gt_affs[1, ...] = gt_labels == gt_labels_tmp[1:, 0:-1]
    ...
    struct = scipy.ndimage.generate_binary_structure(2, 2)
    print(struct.shape, gt_labels.shape)
    for label in np.unique(gt_labels):
        if label == 0:
            continue
    ...
    imsave(out_fn + ".png", np.squeeze(gt_labels, axis=0))
