...
# Copied from https://github.com/jakeret/tf_unet
...
class BaseDataProvider(object):
    ...
    def __init__(self, a_min=None, a_max=None, binary=False):
    ...
    def __call__(self, n):
        ...
        # heads up: change to channels-first
        X = np.zeros((n, self.channels, nx, ny))
        if self.binary:
            Y = np.zeros((n, 1, nx, ny))
        else:
            Y = np.zeros((n, self.n_class, nx, ny))

        X[0] = self.reformat_data(train_data)
        Y[0] = self.reformat_label(labels)

        for i in range(1, n):
            train_data, labels = self._load_data_and_label()
            X[i] = self.reformat_data(train_data)
            Y[i] = self.reformat_label(labels)
...
class ImageDataProvider(BaseDataProvider):
    ...
    def _find_data_files(self, search_path):
        ...
        return [name for name in all_files if
                self.data_suffix in name and not self.mask_suffix in name]
